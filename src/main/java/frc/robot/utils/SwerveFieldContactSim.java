...
public class SwerveFieldContactSim {
  ...
  private static final double HALF_COLLISION_DIST =
      Dimensions.BUMPER_COLLISION_DISTANCE.in(Meters) / 2.0;
  ...
  private final StructArrayPublisher<Pose2d> cornerPosePublisher =
      NetworkTableInstance.getDefault()
          .getStructArrayTopic("Sim/Swerve/CornerPoses", Pose2d.struct)
          .publish();
  private final StructArrayPublisher<Pose2d> edgePosePublisher =
      NetworkTableInstance.getDefault()
          .getStructArrayTopic("Sim/Swerve/EdgePoses", Pose2d.struct)
          .publish();
  ...
  public void handleSwerveSimFieldCollisions() {
    ...
    double frontOffset =
        (isIntakeDeployed != null && isIntakeDeployed.get()) ? INTAKE_DEPLOYED_OFFSET : 0.0;

    cornerBuffer[0] =
        currentSimPose.transformBy(
            new Transform2d(DIAGONAL_OFFSET + frontOffset, DIAGONAL_LATERAL, Rotation2d.kZero));
    cornerBuffer[1] =
        currentSimPose.transformBy(
            new Transform2d(DIAGONAL_OFFSET + frontOffset, -DIAGONAL_LATERAL, Rotation2d.kZero));
    cornerBuffer[2] =
        currentSimPose.transformBy(
            new Transform2d(-DIAGONAL_OFFSET, DIAGONAL_LATERAL, Rotation2d.kZero));
    cornerBuffer[3] =
        currentSimPose.transformBy(
            new Transform2d(-DIAGONAL_OFFSET, -DIAGONAL_LATERAL, Rotation2d.kZero));
  ...
  private boolean[] checkFieldCollisionWithDirections() {
    ...
    if (cornerCollisionFlags[0] && !cornerCollisionFlags[1] && !cornerCollisionFlags[2])
      ccwCollided = true;
    if (cornerCollisionFlags[1] && !cornerCollisionFlags[0] && !cornerCollisionFlags[3])
      cwCollided = true;
    if (cornerCollisionFlags[2] && !cornerCollisionFlags[3] && !cornerCollisionFlags[0])
      cwCollided = true;
    if (cornerCollisionFlags[3] && !cornerCollisionFlags[2] && !cornerCollisionFlags[1])
      ccwCollided = true;
    ...
    return new boolean[] {
      xMinusCollided, xPlusCollided, yMinusCollided, yPlusCollided, ccwCollided, cwCollided
    };
  }
