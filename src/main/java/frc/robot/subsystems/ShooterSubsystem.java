 ...
 *
 * <p>Owns flywheel, hood, and turret motor control; exposes high-level actions used by {@code
 * TheMachine}; publishes telemetry; and maintains simulation behavior.
 */
public class ShooterSubsystem extends SubsystemBase {

  // Runtime dashboard override used for pit/debug operation.
  private final BooleanTopic manualOverrideTopic =
      NetworkTableInstance.getDefault().getBooleanTopic("Conf/TheMachine/ManualOverrideEnabled");
  ...
  private static final double MIN_HOOD_DEG =
      ShooterConstants.MIN_HOOD_ANGLE.in(edu.wpi.first.units.Units.Degrees);
  private static final double MAX_HOOD_DEG =
      ShooterConstants.MAX_HOOD_ANGLE.in(edu.wpi.first.units.Units.Degrees);
  private static final double MIN_TURRET_DEG =
      ShooterConstants.MIN_TURRET_ANGLE.in(edu.wpi.first.units.Units.Degrees);
  private static final double MAX_TURRET_DEG =
      ShooterConstants.MAX_TURRET_ANGLE.in(edu.wpi.first.units.Units.Degrees);
  ...
  private SysIdRoutine createSysIdRoutine(
      String logPrefix, TalonFX motor, double dynamicStepVolts) {
  ...
  private void applyConfig(
      TalonFX motor, com.ctre.phoenix6.configs.TalonFXConfiguration config, String name) {
    ...
    if (!status.isOK()) {
      System.out.println(
          "Could not apply configs to " + name + ", error code: " + status.toString());
    }
  }
  ...
  private void setHoodAngle(double angleInputDegrees) {
    ...
    hoodMotor.setControl(
        hoodPositionControl.withPosition(
            hoodGoalAngle / 360.0 * ShooterConstants.HOOD_GEAR_REDUCTION));
  }
  ...
  public double setTurretAngleDegrees(double requestedAngleDegrees) {
    ...
    tempAngleErrorDeg =
        Math.abs(normalizeToMinus180To180(tempWrappedTargetDeg - tempCurrentAngleDeg));

    // Use more aggressive PID slot near target for tighter final convergence.
    tempTurretPidSlot =
        tempAngleErrorDeg <= ShooterConstants.TURRET_SMALL_ERROR_THRESHOLD_DEG
    ...
    turretMotor.setControl(
        turretPositionControl.withSlot(tempTurretPidSlot).withPosition(tempMotorPosition));
  ...
  private double turretFrameToRobotFrameDeg(double turretFrameDeg) {
    return normalizeToMinus180To180(
        turretFrameDeg + ShooterConstants.TURRET_ZERO_IN_ROBOT_FRAME_DEG);
  }

  private double robotFrameToTurretFrameDeg(double robotFrameDeg) {
    return normalizeToMinus180To180(
        robotFrameDeg - ShooterConstants.TURRET_ZERO_IN_ROBOT_FRAME_DEG);
  }
  ...
  public void syncTurretMotorToAbsoluteEncoder() {
    ...
    tempAbsoluteTurretDeg =
        tempAbsoluteEncoderRot * ShooterConstants.TURRET_ABSOLUTE_DEGREES_PER_ENCODER_ROTATION
  ...
  private static final double FLYWHEEL_ERROR_RPS =
      ShooterConstants.FLYWHEEL_ALLOWABLE_ERROR.in(RotationsPerSecond);
  private static final double HOOD_ERROR_DEG =
      ShooterConstants.HOOD_ALLOWABLE_ERROR.in(edu.wpi.first.units.Units.Degrees);
  private static final double TURRET_ERROR_DEG =
  ...
  private static final double FLYWHEEL_LATCH_DISENGAGE_ERROR_RPS =
      ShooterConstants.FLYWHEEL_ALLOWABLE_ERROR_UNLATCH.in(RotationsPerSecond);
  ...
  private static final double TURRET_LATCH_ENGAGE_SPEED_RPS =
      ShooterConstants.TURRET_ALLOWABLE_SPEED_TO_SHOOT.in(RotationsPerSecond);
  private static final double TURRET_LATCH_DISENGAGE_SPEED_RPS =
      TURRET_LATCH_ENGAGE_SPEED_RPS * 1.5;
  ...
  private void resetReadyLatchesIfGoalChanged() {
    ...
    tempFlywheelSetpointDeltaRps =
        Math.abs(tempFlywheelGoalAbsRps - flywheelLatchGoalVelocityAbsRps);
    ...
    tempTurretSetpointDeltaDeg =
        Math.abs(normalizeToMinus180To180(turretGoalAngleDegrees - turretLatchGoalAngleDegrees));
  ...
  public void publishTelemetry() {
    ...
    SmartDashboard.putNumber(
        "Shooter/HoodMotorPositionRot",
        TelemetryConstants.roundTelemetry(hoodPositionSignal.getValueAsDouble()));
    SmartDashboard.putNumber(
        "Shooter/HoodMotorVelocityRps",
    ...
    SmartDashboard.putNumber(
        "Shooter/TurretMotorPositionRot",
        TelemetryConstants.roundTelemetry(turretPositionSignal.getValueAsDouble()));
    SmartDashboard.putNumber(
        "Shooter/TurretMotorVelocityRps",
    ...
    SmartDashboard.putNumber(
        "Shooter/FlywheelRPS", TelemetryConstants.roundTelemetry(tempFlywheelRps));
    SmartDashboard.putNumber(
        "Shooter/FlywheelGoalRPS", TelemetryConstants.roundTelemetry(flywheelGoalVelocity));
    SmartDashboard.putNumber(
        "Shooter/HoodAngleRot", TelemetryConstants.roundTelemetry(getHoodPosition()));
    SmartDashboard.putNumber(
        "Shooter/HoodAngleDeg", TelemetryConstants.roundTelemetry(tempHoodAngleDeg));
    SmartDashboard.putNumber(
        "Shooter/HoodGoalDeg", TelemetryConstants.roundTelemetry(hoodGoalAngle));
    SmartDashboard.putNumber(
        "Shooter/TurretAngleDeg", TelemetryConstants.roundTelemetry(tempTurretAngleClamped));
    SmartDashboard.putNumber(
        "Shooter/TurretGoalDeg", TelemetryConstants.roundTelemetry(turretGoalAngleDegrees));
    SmartDashboard.putNumber(
        "Shooter/FlywheelRpmError", TelemetryConstants.roundTelemetry(tempFlywheelRpmError));
    SmartDashboard.putNumber(
        "Shooter/HoodDegError", TelemetryConstants.roundTelemetry(tempHoodDegError));
    SmartDashboard.putNumber(
        "Shooter/TurretDegError", TelemetryConstants.roundTelemetry(tempTurretDegError));
  ...
  public void simUpdate() {
    if (!isSimulationInitialized) {
      ...
      hoodSimState.setRawRotorPosition(
          Rotations.of(
              hoodSim.getAngleRads() / (2 * Math.PI) * ShooterConstants.HOOD_GEAR_REDUCTION));
      hoodSimState.setRotorVelocity(
          RadiansPerSecond.of(hoodSim.getVelocityRadPerSec())
              .times(ShooterConstants.HOOD_GEAR_REDUCTION));
  ...
  double turretCamFrcForward =
      TheMachineConstants.TURRET_LL_POSE.getX() + TheMachineConstants.SHOOTER_ROTATION_AXIS.getX();
  double turretCamFrcLeft =
      TheMachineConstants.TURRET_LL_POSE.getY() + TheMachineConstants.SHOOTER_ROTATION_AXIS.getY();
  ...
  double turretLLUp =
      TheMachineConstants.TURRET_LL_POSE.getZ() + TheMachineConstants.SHOOTER_ROTATION_AXIS.getZ();
  double turretLLPitchDegrees =
      Math.toDegrees(TheMachineConstants.TURRET_LL_POSE.getRotation().getY());
  ...
  public void periodic() {
    ...
    turretCamFrcForward =
        TheMachineConstants.TURRET_LL_POSE.getX() * Math.cos(turretLLHeading)
            - TheMachineConstants.TURRET_LL_POSE.getY() * Math.sin(turretLLHeading);
    turretCamFrcLeft =
        TheMachineConstants.TURRET_LL_POSE.getX() * Math.sin(turretLLHeading)
    ...
    SmartDashboard.putNumber(
        "Throughbore", TelemetryConstants.roundTelemetry(turretAbsoluteEncoder.get()));
    SmartDashboard.putNumber(
        "TurretLLHeading", TelemetryConstants.roundTelemetry(Math.toDegrees(turretLLHeading)));
